...
# This is an array of where to start looking for space for a free block of a
# particular size.
my @minLocation;

# For each file, try to move it into free space with a lower block idx.
for my $file (reverse @fileMap) {
  for my $j ($minLocation[$file->[1]] .. $#freeMap) {
    my $free = $freeMap[$j];
    ...
    # Update the minLocation table.
    for my $k ($file->[1]..9) {
      $minLocation[$k] = $j if $minLocation[$k] < $j;
    }
  }
}
